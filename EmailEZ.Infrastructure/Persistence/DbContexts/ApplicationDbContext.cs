...
public class ApplicationDbContext : DbContext, IApplicationDbContext
{
    ...
    public DbSet<WorkspaceApiKey> WorkspaceApiKeys { get; set; } = null!;
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Email>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<EmailAttachment>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<EmailEvent>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<AuditLog>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<EmailConfiguration>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<WorkspaceUser>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        modelBuilder.Entity<WorkspaceApiKey>().HasQueryFilter(e => !e.IsDeleted && (!_workspaceContext.WorkspaceId.HasValue || e.WorkspaceId == _workspaceContext.WorkspaceId));
        ...
        ConfigureWorkspaceUser(modelBuilder.Entity<WorkspaceUser>());
        ConfigureWorkspaceApiKey(modelBuilder.Entity<WorkspaceApiKey>());
    ...
    private static void ConfigureEmail(EntityTypeBuilder<Email> entity)
    {
        ...
        entity.HasOne(e => e.Workspace)
              ...
              .OnDelete(DeleteBehavior.Restrict); // Prevent deleting workspace if emails exist
        ...
        entity.HasIndex(e => e.WorkspaceId); // For efficient workspace-specific email queries
    ...
    private static void ConfigureEmailAttachment(EntityTypeBuilder<EmailAttachment> entity)
    {
        ...
        entity.HasIndex(ea => ea.WorkspaceId); // For denormalized workspace-specific queries
    }

    private static void ConfigureEmailEvent(EntityTypeBuilder<EmailEvent> entity)
    {
        ...
        entity.HasIndex(ee => ee.WorkspaceId);
    ...
    private static void ConfigureAuditLog(EntityTypeBuilder<AuditLog> entity)
    {
        ...
        entity.HasOne(al => al.Workspace)
              .WithMany(t => t.AuditLogs) // Ensure this navigation property exists on Workspace if you want it
              .HasForeignKey(al => al.WorkspaceId)
              .IsRequired(false) // WorkspaceId is nullable
              .OnDelete(DeleteBehavior.Restrict); // Prevent deleting workspace if audit logs exist
        ...
        entity.HasIndex(al => al.WorkspaceId);
        entity.HasIndex(al => al.EventType);
        entity.HasIndex(al => al.CreatedAt); // Using BaseEntity.CreatedAt for audit timestamp
    }

    private static void ConfigureEmailConfiguration(EntityTypeBuilder<EmailConfiguration> entity)
    {
        entity.HasKey(e => e.Id); // Define primary key if not covered by BaseAuditableEntity

        // Configure the one-to-many relationship with Workspace
        entity.HasOne(e => e.Workspace)             // An EmailConfiguration has one Workspace
                .WithMany(w => w.EmailConfigurations)                        // A Workspace can have many EmailConfigurations
                .HasForeignKey(e => e.WorkspaceId)    // EmailConfiguration.WorkspaceId is the foreign key
                .IsRequired()                      // An EmailConfiguration must be linked to a Workspace
                .OnDelete(DeleteBehavior.Cascade); // If a Workspace is deleted, delete its EmailConfigurations

        // You might want to make SmtpHost + Username unique per Workspace if only one config per workspace is allowed
        // entity.HasIndex(e => new { e.WorkspaceId, e.SmtpHost, e.Username }).IsUnique();

    }

    private static void ConfigureWorkspaceUser(EntityTypeBuilder<WorkspaceUser> entity)
    {
        entity.HasKey(wu => wu.Id); // Define primary key if not covered by BaseAuditableEntity
        // Configure the many-to-many relationship with Workspace
        entity.HasOne(wu => wu.Workspace) // A WorkspaceUser belongs to one Workspace
              .WithMany(w => w.WorkspaceUsers) // A Workspace can have many WorkspaceUsers
              .HasForeignKey(wu => wu.WorkspaceId) // Foreign key in WorkspaceUser
              .OnDelete(DeleteBehavior.Cascade); // If a Workspace is deleted, delete its WorkspaceUsers

        entity.HasIndex(entity => new { entity.WorkspaceId, entity.UserId })
            .IsUnique(); // Ensure a user can only be linked to a workspace once


    }

    private static void ConfigureWorkspaceApiKey(EntityTypeBuilder<WorkspaceApiKey> entity)
    {
        entity.HasKey(wak => wak.Id); // Define primary key if not covered by BaseAuditableEntity

        // Configure the many-to-one relationship with WorkspaceUser
        entity.HasOne(wak => wak.WorkspaceUser) // A WorkspaceApiKey belongs to one WorkspaceUser
              .WithMany(wu => wu.WorkspaceApiKeys) // A WorkspaceUser can have many API keys
              .HasForeignKey(wak => wak.WorkspaceUserId) // Foreign key in WorkspaceApiKey
              .OnDelete(DeleteBehavior.Cascade); // If a WorkspaceUser is deleted, delete its API keys

        // Configure the many-to-one relationship with Workspace
        entity.HasOne(wak => wak.Workspace) // A WorkspaceApiKey belongs to one Workspace
              .WithMany(w => w.WorkspaceApiKeys) // A Workspace can have many API keys
              .HasForeignKey(wak => wak.WorkspaceId) // Foreign key in WorkspaceApiKey
              .OnDelete(DeleteBehavior.Cascade); // If a Workspace is deleted, delete its API keys

        entity.Property(wak => wak.ApiKeyHash).IsRequired().HasMaxLength(255);
        entity.Property(wak => wak.Name).HasMaxLength(100);
    }
