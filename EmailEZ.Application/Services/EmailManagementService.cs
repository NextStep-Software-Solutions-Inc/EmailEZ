...
public class EmailManagementService : IEmailManagementService
{
    ...
    private readonly ILogger<EmailManagementService> _logger;

    public EmailManagementService(
        IUnitOfWork unitOfWork,
        IEmailSender emailSender,
        IBackgroundJobClient backgroundJobClient,
        ILogger<EmailManagementService> logger)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var emailConfig = await _unitOfWork.EmailConfigurations
            .GetEmailConfigurationByIdAsync(request.EmailConfigurationId, cancellationToken) ?? throw new InvalidOperationException($"Email configuration with ID '{request.EmailConfigurationId}' not found.");
        ...
        if (emailConfig.Workspace == null || !emailConfig.Workspace.IsActive)
        {
            throw new InvalidOperationException($"Workspace '{emailConfig.WorkspaceId}' is not active or does not exist.");
        }
        ...
        var jobId = _backgroundJobClient.Enqueue(() => SendEmailAndLogAsync(emailMessage, emailConfig.Id, createdEmail.Id, null));

        // ? Update email with job ID
        createdEmail.HangfireJobId = jobId;
        _unitOfWork.Emails.Update(createdEmail);
        await _unitOfWork.SaveChangesAsync(cancellationToken);
        ...
        string? jobId)
    {
        ...
        if (email == null)
        {
            _logger.LogError($"Email not found for ID: {emailId}");
            return;
        }

        var config = await _unitOfWork.EmailConfigurations.GetByIdAsync(emailConfigurationId, CancellationToken.None);
        if (config == null)
        {
            _logger.LogError($"Email configuration not found for ID: {emailConfigurationId}");
            return;
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var retryableEmails = await _unitOfWork.Emails.GetWithIncludesAsync(
        ...
        foreach (var email in retryableEmails)
        {
            var emailMessage = new EmailMessage
            {
                ...
                Body = (email.IsHtml ? email.BodyHtml  : email.BodyPlainText) ?? string.Empty,
                IsHtml = email.IsHtml
            };

            // Re-queue the email
            var jobId = _backgroundJobClient.Enqueue(() => SendEmailAndLogAsync(emailMessage, email.EmailConfiguration.Id, email.Id, null));
